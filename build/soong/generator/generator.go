...
func init() {
	android.RegisterModuleType("aosmp_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aosmpExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aosmpExpandVariables(ctx, dep_file)
	...
	cmd := aosmpExpandVariables(ctx, String(g.properties.Cmd))
